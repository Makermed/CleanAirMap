    ...
    definition(t) {
        t.field('addReadingNewLocation', {
            ...
            async resolve(_root, args, ctx) {
                const locationModel = {
                    name: args.input.location.name,
                    street: args.input.location.street,
                    locality: args.input.location.locality,
                    place: args.input.location.place,
                    district: args.input.location.district,
                    region: args.input.location.region,
                    postcode: args.input.location.postcode,
                    country: args.input.location.country,
                    type: args.input.location.type,
                    latitude: args.input.location.position.latitude,
                    longitude: args.input.location.position.longitude,
                    description: args.input.location.description,
                    avgCo2: null,
                    created_id: 1
                }
                //@ts-ignore: missing ID is okay, as it will be autogenerated
                //TODO: perhaps create a NewLocation type with no ID
                const newLocation = await ctx.db.locationDAO.create ( locationModel )
                if (newLocation === null) {
