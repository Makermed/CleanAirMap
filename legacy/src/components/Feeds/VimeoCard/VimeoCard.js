...
class VimeoCard extends React.Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    const likesCnt = parseInt(article.param1);
    const playsCnt = parseInt(article.param2);
    const commentsCnt = parseInt(article.param3);

    const likes   = int2kstring(likesCnt);
    const plays   = int2kstring(playsCnt);
    const comments = int2kstring(commentsCnt);
        ...
        <div className={classes.carddiv}>
          <CardHeader
            ...
            avatar={
              <div>
                ...
                <img
                  alt={"vimeo"}
                  src={`/static/images/icons/${theme_mode}/vimeo.png`}
          ...
          <CardContent className={classes.content} onClick={e => handleClick(article)}>
            {sentences.length > 0 && sentences.map((sentence, index) =>
              <Typography className={classes.detail_txt} key={`sentence-${index}`} variant="body2">
          ...
          <CardActions>
            ...
            >
              ...
              <img
                alt={"plays"}
                src={`/static/images/icons/${theme_mode}/play.png`}
                className={classes.reactionimg}
              />
              <Typography className={classes.reactionvalue}>{plays}</Typography>
              <img
                alt={"comments"}
