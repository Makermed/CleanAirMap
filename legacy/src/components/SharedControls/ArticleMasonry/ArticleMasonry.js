...
class ArticleMasonry extends React.Component {
  constructor(props) {
    super(props);
  ...
  listenToScroll = (event) => {
    ...
    // console.log("winscroll :", winScroll, ", height :", height, ", scrolled :", scrolled);
    this.props.saveScrollPos(window.scrollX, window.scrollY);
    if (scrolled === 0.0) {
      //this.props.saveScrollPos(window.scrollX, window.scrollY);
      ...
      <div className={classes.root} style={{width: width}}>
        ...
        >
          {articles2show.map(article => (
            <div className={classes.article} key={article.nid}>
              <Article
